...
max_episode_steps_per_episode = 10000 # Can be any Value

register(
        id='MyTiagoWall-v0',
        entry_point='openai_ros.task_envs.tiago.tiago_wall:TiagoWallEnv',
        max_episode_steps=max_episode_steps_per_episode,
    )

class TiagoWallEnv(tiago_env.TiagoEnv):
    def __init__(self):
        """
        ...
        self.linear_forward_speed = rospy.get_param('/tiago/linear_forward_speed')
        self.linear_turn_speed = rospy.get_param('/tiago/linear_turn_speed')
        self.angular_speed = rospy.get_param('/tiago/angular_speed')
        ...
        self.desired_point.x = rospy.get_param("/tiago/desired_pose/x")
        self.desired_point.y = rospy.get_param("/tiago/desired_pose/y")
        self.desired_point.z = rospy.get_param("/tiago/desired_pose/z")
        ...
        # Rewards
        self.forwards_reward = rospy.get_param("/tiago/forwards_reward")
        self.turn_reward = rospy.get_param("/tiago/turn_reward")
        self.end_episode_points = rospy.get_param("/tiago/end_episode_points")

        self.cumulated_steps = 0.0

        # Here we will add any init functions prior to starting the MyRobotEnv
        super(TiagoWallEnv, self).__init__()
    ...
    def _is_done(self, observations):
        ...
        if self._episode_done:
            ...
            rospy.logerr("Tiago didnt crash at least ==>")
            ...
            if current_position.x <= MAX_X and current_position.x > MIN_X:
                if current_position.y <= MAX_Y and current_position.y > MIN_Y:
                    rospy.logdebug("Tiago Position is OK ==>["+str(current_position.x)+","+str(current_position.y)+"]")
                    ...
                    rospy.logerr("Tiago to Far in Y Pos ==>"+str(current_position.x))
                    self._episode_done = True
            else:
                rospy.logerr("Tiago to Far in X Pos ==>"+str(current_position.x))
                self._episode_done = True
